...
------- Declare flags vars
DECLARE @STRAIGHT_LINE_CONTRACT_FLG BIT 
...
SELECT @CONTRACT_REG_MTH_AMT=SUM(DiscountedMonthlyPrice)
FROM Staging..LineItem  WHERE ContractID=@CONTRACTID
SELECT @CONTRACT_REG_MTH_AMT CONTRACT_REG_MTH_AMT

--------------------- Step2 calculate the escalation amount (value of escalations )
SELECT @CONTRACT_ESCALATION_AMT=SUM(LE.EscalationAmount)
FROM Staging..LineItemEscalation LE 
     JOIN Staging..LineItem L ON LE.LineItemID=L.LineItemID
                                          WHERE ContractID=@CONTRACTID
...
INSERT [dbo].[StraightLineContracts] ([ContractID],[Waterfall_Amt],[Waterfall_Stub_Amt],[Contract_Final_Amt])
SELECT @CONTRACTID,@WATERFALL_AMT,@WATERFALL_STUB_AMT,@CONTRACT_FINAL_AMT
